...
import type { RecordPaymentInput } from '@somwave/shared';
...
export async function create(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    sendData(
      res,
      await paymentService.recordPayment({ ...body, actorId, idempotencyKey: key }),
      201,
    );
  } catch (err) {
    next(err);
...
export async function listForInvoice(
  ...
  try {
    ...
    sendData(res, await paymentService.listPayments(invoiceId));
  } catch (err) {
    next(err);
  }
}
